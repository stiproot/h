...
import { lastFencedJson, type WorkflowStep } from "workflow-core";
...
): Effect.Effect<{ cronId: string; active: boolean }, WorkflowError, CronStore> =>
  // CronStore alone, not the whole CronScanEnv: arming is an ensure-exists WRITE and touches no
  // invoker or workflow store. The wider requirement was a type-level overstatement, and it kept
  // this seam out of reach of any host that has registries but no fire path — which is exactly what
  // the local executor is at its closing bracket.
  Effect.gen(function* () {
...
// ---------------------------------------------------------------------------
// The arm-at-birth PLAN (§10) — pure, and shared by both hosts
// ---------------------------------------------------------------------------
//
// Lifted out of workflow-svc's register-cron activity on 2026-08-17. It decides WHETHER and
// WITH WHAT to arm a recurrence, and both executors have to decide identically: the Dapr engine
// calls it from its closing bracket, the local executor from its own. A second copy would let
// one substrate arm a revise-pr loop for a PR that never opened while the other declined.
//
// The activity around it stays substrate-side — it is the Dapr binding, not the decision.
