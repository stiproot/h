...
/**
 * A denied-executor entry with PROVENANCE: `operator`
 ...
 * `cost-budget` entries are the watcher's third kind:
 ...
 *. `denied` holds entries (above) OR bare
...
export const ExecPolicy = Schema.Struct({
  ...
  // Per-executor daily cost budgets in USD, keyed by SHORTNAME (the cost-containment
