...
- **Compose env precedence (shell exports shadow `.env`)** — compose interpolates `${VAR}` from the process environment FIRST, the project `.env` file second. The user profile exports `GH_TOKEN`, so after editing a secret in `.env` a plain `docker compose … --force-recreate` silently stamps the OLD exported value back into the containers. Recreate with `env -u GH_TOKEN docker compose … up -d --force-recreate` (or verify with `docker exec <ctr> printenv GH_TOKEN` against `.env` before debugging "the token doesn't work"). Applies to every var in `.env`, not just `GH_TOKEN`. Bit us live 2026-07-16 (two stale recreates of claude-agent).
