...
These tools return an error STRING rather than raising: they are handed to the model, and a
raised exception would abort the run instead of letting the agent see the refusal and adjust.
"""

from pathlib import Path

from infrastructure.tools import make_tools


def _call(cwd: Path, name: str, **kwargs) -> str:
    return make_tools(cwd)[name].func(**kwargs)
...
def test_read_skill_refuses_a_traversal_skill_name(tmp_path):
    out = _call(tmp_path, "read_skill", skill_name="../../etc")
