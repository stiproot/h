...
import { PiRunnerLive } from "./pi-runner.ts";
...
function makeHarness(invoke: Invoke): {
  ...
  const baseDir = mkdtempSync(join(tmpdir(), "pi-base-"));
  const runsDir = mkdtempSync(join(tmpdir(), "pi-runs-"));
  const logs: Record<string, unknown>[] = [];
  const layer = PiRunnerLive.pipe(
    Layer.provide(
      Layer.mergeAll(
...
describe("PiRunnerLive", () => {
  it("maps the invocation result onto the AgentResponse and writes the run ledger", async () => {
    ...
    expect(response.runId).toContain("wf-1:pi-agent:");
    ...
    expect(eventLogs[0]).toMatchObject({ agent: "pi", workflowInstanceId: "wf-1" });

    await runtime.dispose();
  });

  it("maps an invoker failure to AgentRunError (wrapping PiRunError) and records a failed run", async () => {
    ...
    if (Exit.isFailure(exit)) {
      ...
      expect(error.agentId).toBe("pi-agent");
      const inner = error.cause as { _tag: string; cause: unknown };
      expect(inner._tag).toBe("PiRunError");
