...
- 2026-07-24 — **Container app-server blocker fixed (issue #59).** Root cause: codex 0.145's "in-process app-server" writes state into the root-owned npm global package dir (`/usr/lib/node_modules/@openai/codex`), which non-root users can't write → EACCES (os error 13, fatal). The PATH-aliases failure (EPERM, os error 1) is the same class: `fs.protected_hardlinks=1` blocks hardlinking a root-owned binary. Fix: two lines appended to the Dockerfile's first `RUN` block after `npm install -g @openai/codex`: `chown -R agent-cli:agent $(npm root -g)/@openai/codex` (agent-cli becomes the owner, can init the app-server) + `chmod -R g+w $(npm root -g)/@openai/codex` (agent-svc/same-group write for the repro path). Container stays non-root; no other changes.
