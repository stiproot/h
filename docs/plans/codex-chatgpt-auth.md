...
- 2026-07-23 — **Container e2e: 4 wiring bugs fixed, 1 deep blocker remains.** Ran the full inline codex chain in compose. Each failure surfaced a real codex-agent compose gap (all pre-existing from PR #53's under-wired service, never caught because codex had never run in a container): (1) `GH_TOKEN` missing → broke worktree fetch + create-pr push + the github MCP bearer (`43f52cf`); (2) `H_SKILLS_DIR` + `./skills:/h-skills` mount missing → setup's `cp -r $H_SKILLS_DIR/. …` became `cp -r /. …` (whole root fs) (`56c3e5f`); (3) runner-written `config.toml` not group-accessible → the codex CLI (dropped sub-agent uid) got "config.toml: Permission denied" → chmod 0660 like the seeded auth.json (`79f95f4`). **BLOCKER (not fixed):** codex 0.145's "in-process app-server" + "PATH aliases" fail for ANY non-root user in the container — `could not create PATH aliases: Operation not permitted (os error 1)` (EPERM, the protected-hardlinks class — codex hardlinks its root-owned binary at /usr/lib/node_modules) + `failed to initialize in-process app-server client: Permission denied (os error 13)`. Reproduces as BOTH agent-svc (10001, undropped) AND agent-cli (10002, dropped), independent of HOME — so it's not the privilege drop, it's codex being root-installed and needing to create helper binaries / app-server state as non-root. **Container e2e paused here** (local e2e already fully validated codex end-to-end — PR #58). Investigation paths for the follow-up: pin an older codex without the app-server requirement; a codex flag to disable/relocate the app-server + PATH aliases; install codex per-user (writable) rather than root-global; or a writable helper-bin/runtime dir. Tracked as an h issue.
- 2026-07-24 — **Container app-server blocker RESOLVED (issue #59, real fix `cac58f6`).** The blocker was NOT the npm package (an h feature-run's PR #60 guessed that and was validated wrong in a container — closed). Real cause via `RUST_LOG`: codex's app-server writes a SQLite state DB into CODEX_HOME, and pointing CODEX_HOME at the host-shared `/workspace/codex-home` polluted it with cross-uid state files (host uid 1000 wrote them 0644 → the container user couldn't write → "readonly database" → fatal). Fix = **isolate CODEX_HOME**: a container-private `/codex-home` (Dockerfile, setgid group-writable) that only the container's own users touch; compose points CODEX_HOME there + mounts the host auth.json read-only as `CODEX_SEED_AUTH`; the runner seeds auth in (0660) + rm-rewrites config.toml so re-provisioning survives codex's own [projects]-trust rewrites. Validated: a full codex-agent container `/run` on a ChatGPT plan returned `CONTAINER-FIX-OK`. Host mode unaffected (CODEX_SEED_AUTH unset). **Container mode now works end-to-end** (auth + MCP + state), matching host mode — the container e2e chain can be re-run. Process note: the h loop produced a plausible-but-wrong fix (#60); the human container-validation step caught it before merge — the intended guardrail working.
