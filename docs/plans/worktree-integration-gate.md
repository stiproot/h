...
- **Phase 1 — the test.**
  - *1a — substrate*: kustomize base from the existing manifests (untouched) + generated
    per-run overlay; sidecar injection verified in an ephemeral namespace; secrets not
    required by the slice.
  - *1b — smoke*: `stub-agent` app + its declared contract; smoke workflow checked in;
    `scripts/itest/run-itest.sh` with the exit-code taxonomy; `make itest` green by hand.
  - *1c — failure honesty*: red on a seeded workflow-svc breakage (prove it can fail, with
    `assertion` class); `make itest-gc` sweeper; evidence dir + ledger wiring.
- **Phase 2 — the gate.** `run-itest` activity (base-ref harness, timeout, taxonomy, tree
  hash; unit tests + activity-registry entry); `run-itest.tmpl.yaml` overlay + template gate +
  goldens; `itest.skip` break-glass; recompose `implement-pr` (after the activity deploys);
  watch-budget bump.
