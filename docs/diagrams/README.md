...
| [execution-substrates-c4-container](./execution-substrates-c4-container.md) | C4 container | One composition, two executors: the shared `h` CLI front door, the DIRECT substrate (runner + agent CLI children, no infrastructure) beside the SERVICE substrate, what `workflow-core` makes structurally symmetric, and the edge by which a direct agent fires durable work |
| [direct-run-sequence](./direct-run-sequence.md) | sequence | One `--direct` run end to end: render → job on stdin → step walk (token resolution, refused activities, worktree, setup-skip, agent spawn, contract validation) → envelope; plus the Ctrl-C reap path |
