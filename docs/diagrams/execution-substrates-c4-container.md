...
C4Container
  ...
  Container_Boundary(localsub, "LOCAL substrate — the operator's own process") {
    Container(runner, "h-local runner", "TypeScript/Node.js", "Walks the steps in-process. No registries, no engines.")
...
- **The local boundary contains no infrastructure — that IS the substrate.** No Dapr sidecar, no
  ...
  in-process, the driver IS the supervisor. So `--local` refuses `--cron/--watch/--budget/--retry/
