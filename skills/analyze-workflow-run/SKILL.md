...
| Follow-up | What the run set up next: a per-PR revise cron (implement-pr's `arm-revise-pr`) or another seeded task | cron registry `cron:sub:*` / `cron:discover:*` (via `h cron list` or `state_get`); statestore `tasks:index` for pub/sub-seeded tasks |
...
5. **Check the loop** (§7): did the run set up its expected continuation — e.g. a `implement-pr` that
